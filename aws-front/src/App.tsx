...
export function App() {
  ...
  async function fetchMoedas() {
    try {
      const res = await axios.get<Moeda[]>("http://52.20.178.218:5000/moedas");
  ...
  async function criarMoeda() {
    ...
    try {
      await axios.post("http://52.20.178.218:5000/moedas", { nome: nomeCriar.trim(), valor: valorNum });
      setOpenCriar(false);
      setNomeCriar("");
      setValorCriar("");
      fetchMoedas();
  ...
  async function editarMoeda() {
    ...
    try {
      await axios.put(`http://52.20.178.218:5000/moedas/${nomeEditar.trim()}`, { valor: valorNum });
      setOpenEditar(false);
      setNomeEditar("");
      setValorEditar("");
      fetchMoedas();
  ...
  async function deletarMoeda() {
    ...
    try {
      await axios.delete(`http://52.20.178.218:5000/moedas/${nomeDeletar.trim()}`);
